...
// Reset inputs when creating new room
socket.on("reset-inputs", ({ playerA, playerB }) => {
...
// Clear buzzer winner display on display screen
function clearBuzzerWinner() {
  socket.emit("clear-buzzer-winner");
}

// ===== Import: Tab Switch =====
function switchTab(tab) {
  document
    .getElementById("tabSheet")
    .classList.toggle("active", tab === "sheet");
  document
    .getElementById("tabExcel")
    .classList.toggle("active", tab === "excel");
  document
    .getElementById("sheetContent")
    .classList.toggle("hidden", tab !== "sheet");
  document
    .getElementById("excelContent")
    .classList.toggle("hidden", tab !== "excel");
  // Save active tab
...
async function importSheet() {
  ...
  try {
    ...
    if (!csvUrl) {
      showImportStatus(
        "Link không hợp lệ. Hãy dùng link Google Sheets public.",
        "error",
      );
      return;
    }
    ...
    if (!response.ok)
      throw new Error("Không thể tải sheet. Hãy đảm bảo sheet được public.");
...
function handleFileSelect(event) {
  ...
  if (selectedFiles.length > 0) {
    ...
    filesDiv.innerHTML = selectedFiles
      .map(
        (f, i) =>
    ...
    label.querySelector("span").textContent =
      "📁 Chọn file Excel / CSV (nhiều file)";
    filesDiv.innerHTML = "";
  }
}
...
async function uploadExcel() {
  ...
  try {
    ...
    if (data.success) {
      ...
      document.getElementById("fileLabel").querySelector("span").textContent =
        "📁 Chọn file Excel / CSV (nhiều file)";
...
function fullResetGame() {
  if (
    !confirm(
      "⚠️ Reset toàn bộ?\nĐiểm số, câu hỏi, mã phòng, tên người chơi — tất cả sẽ bị xóa sạch.",
    )
  )
    return;
  if (
    !confirm(
      "🔴 XÁC NHẬN LẦN 2: Bạn thực sự muốn XÓA SẠCH tất cả dữ liệu? Không thể hoàn tác!",
    )
  )
    return;
...
socket.on("room-status", ({ slots }) => {
  ...
  slot0.textContent = slots[0]
    ? `✅ ${nameA}: đã vào`
    : `⬜ ${nameA}: chưa vào`;
  slot0.className = "slot" + (slots[0] ? " slot-filled" : "");
  slot1.textContent = slots[1]
    ? `✅ ${nameB}: đã vào`
    : `⬜ ${nameB}: chưa vào`;
...
function renderQuestionSets(state) {
  ...
  sets.forEach((set) => {
    ...
    const sourceName =
      set.source.length > 40 ? set.source.substring(0, 40) + "..." : set.source;
